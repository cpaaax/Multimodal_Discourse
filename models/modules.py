...
class Attention(nn.Module):
    def __init__(self, hidden_size):
        ...
        self.h2att = nn.Linear(hidden_size, hidden_size)
        self.feat2att = nn.Linear(hidden_size, hidden_size)

        self.alpha_net = nn.Linear(hidden_size, 1)

        self.att_hid_size = hidden_size
    def forward(self, h, att_feats, mask=None):
        # The p_att_feats here is already projected
        att_size = att_feats.numel() // att_feats.size(0) // att_feats.size(-1)
        p_att_feats = self.feat2att(att_feats)
        att = p_att_feats.view(-1, att_size, self.att_hid_size)
        ...
        weight = F.softmax(dot, dim=1)  # batch * att_size
        if mask is not None:
            weight = weight * mask.view(-1, att_size).to(weight)
            weight = weight / weight.sum(1, keepdim=True)  # normalize to 1
        att_feats_ = att_feats.view(-1, att_size, att_feats.size(-1))  # batch * att_size * att_feat_size
        att_res = torch.bmm(weight.unsqueeze(1), att_feats_).squeeze(1)  # batch * att_feat_size
...
class CoAttention(nn.Module):
    def __init__(self, text_feat_size, img_feat_size):
        """Initialize model."""
